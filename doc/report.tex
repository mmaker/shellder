...
\item \code{process.[ch]} nella quale viene definito il tipo \texttt{proc\_t} e le funzioni
...
cicli di scrittura potenzialmente fastidiosi, specialmente in caso di bug.
...
La valutazione delle costanti \`e un task La scelta delle costanti dello scheduler pu\`o risultare intuitiva se si adnalizzano i casi limite:
\begin{itemize}
...
\item l'intera coda dei prcessi viene gestita tramite un vettore di grandezza \code{HEAPLEN}
      ...
      gestibili in background. Possibili soluzioni sarebbero l'implementazione nel metodo
      \code{pinsert()} di una chiamata alla system-call \code{resize()} per una riallocazione
